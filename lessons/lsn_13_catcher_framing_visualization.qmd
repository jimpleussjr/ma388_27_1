...
1. Import and merge Statcast pitch data with player ID maps.
2. Create faceted visualizations to compare called-strike proportions across catchers.
3. Discuss challenges in isolating catcher framing ability from confounding factors.
4. Calculate called-strike proportions for pitches near the edge of the strike zone.
...
```{r}
pitches_taken |> 
  mutate(close_pitch = case_when(
    between(plate_x,-plate_width/12/2-,plate_width/12/2+2) &
      between(plate_z,sz_top-2,sz_top+2) ~ TRUE)
