...
1. Fit a generalized additive model (GAM) to model called-strike probability as a smooth function of pitch location.
2. Use a GAM to adjust for pitch location when comparing catchers.
3. Extend a two-catcher comparison to consider modeling many catchers simultaneously.
...
pitches_taken_subset$pred <- predict(linear_fit, newdata = pitches_taken_subset, type = "response")
...
How does our prediction fit align with the actual balls and strikes?
...
p1<-ggplot() +
  geom_point(
    ...
    # aes(x = plate_x, y = plate_z, color = pred),
    aes(x = plate_x, y = plate_z, color = factor(called_strike)),
    alpha = 0.3
  ) +
  coord_equal() +
  labs(
    title = "Actual Strikes",
    color = "Called Strike"
  ) +
  theme_minimal()
