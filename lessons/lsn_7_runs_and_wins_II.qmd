...
1. Compare three models (linear, Pythagorean with k=2, Pythagorean with estimated k) for predicting win percentage.
2. Derive and compute incremental runs per win under the Pythagorean model.
3. Use RMSE to evaluate and compare model performance.
4. Estimate the Pythagorean exponent via log-log regression without an intercept.
...
[Check out the Bill James shoutout in Moneyball](https://www.youtube.com/watch?v=j59yEKztcJ0)
...
Do we see any pattern? Can we check to see if there's some systematic bias in the model that we chose? What would that even look like in the residuals?
